...
--
-- 테이블 접두어: voice_ (wow-series 공유 DB 규칙)
-- vote_ 접두어는 미래 WOWVote(투표 앱)용으로 예약됨
-- ============================================================


-- ------------------------------------------------------------
-- 1) voice_sessions — 행사 단위 관리 (A는 'default' 1개만 사용)
-- ------------------------------------------------------------
...
CREATE TABLE voice_questions (
  ...
  likes_count INTEGER DEFAULT 0,           -- 좋아요 수 (voice_likes 테이블과 동기화)
...
CREATE INDEX idx_voice_questions_session_likes
  ON voice_questions(session_id, likes_count DESC, created_at DESC);


-- ------------------------------------------------------------
-- 3) voice_likes — 중복 좋아요 방지
-- ------------------------------------------------------------
...
CREATE TABLE voice_likes (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  question_id       UUID NOT NULL REFERENCES voice_questions(id) ON DELETE CASCADE,
...
-- ------------------------------------------------------------
INSERT INTO voice_sessions (code, title, host_name)
...
ALTER TABLE voice_questions ENABLE ROW LEVEL SECURITY;
ALTER TABLE voice_likes     ENABLE ROW LEVEL SECURITY;
...
-- ------------------------------------------------------------
-- 누구나 읽기 가능
CREATE POLICY "select_all_sessions"  ON voice_sessions  FOR SELECT USING (true);
CREATE POLICY "select_all_questions" ON voice_questions FOR SELECT USING (true);
CREATE POLICY "select_all_likes"     ON voice_likes     FOR SELECT USING (true);

-- 질문 작성 가능 (익명 청중)
CREATE POLICY "insert_questions" ON voice_questions FOR INSERT WITH CHECK (true);

-- 좋아요 작성 가능 (익명 청중)
CREATE POLICY "insert_likes" ON voice_likes FOR INSERT WITH CHECK (true);

-- 질문 UPDATE 허용 (A에서는 likes_count 증가에만 사용)
-- ⚠️ B에서 호스트 인증 도입 시, 이 정책을 좁혀서
--    is_hidden/is_answered는 호스트만 바꾸도록 분리할 예정.
CREATE POLICY "update_question_likes" ON voice_questions FOR UPDATE USING (true);

-- ============================================================
-- 완료! 확인용 쿼리:
--   SELECT * FROM voice_sessions;                       -- 1행(default) 나오면 OK
